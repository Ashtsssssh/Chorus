...
import { submitJob, getJob } from '../api/api.js';
...
if (isNaN(MAX_FILE_SIZE)) console.warn(`${LOG} ⚠️  VITE_MAX_FILE_SIZE is not set or not a number. File size validation is disabled.`);
if (isNaN(MAX_TOTAL_SIZE)) console.warn(`${LOG} ⚠️  VITE_MAX_TOTAL_SIZE is not set or not a number. Total size validation is disabled.`);
...
export default function Submitter({ user, onJobSubmitted }) {
  ...
  useEffect(() => {
    ...
    // intentionally mounts -> unmounts -> remounts once in dev mode to
    // surface effect bugs. The cleanup below sets this to false on that
    // synthetic unmount; without resetting it here, it stays false
    // forever even though the component is actually mounted and visible,
    // silently killing every `if (!isMountedRef.current) return;` guard
  ...
  const handleSubmit = async (e) => {
    ...
    if (validationError) {
      console.warn(`${LOG} Validation failed: ${validationError}`);
    ...
    try {
      ...
      console.log(`${LOG} 📚 Reading chunker script...`);
      const chunkerCode = await readFileAsText(chunkerFile);
      console.log(`${LOG} ✅ Chunker script loaded (${chunkerCode.length} chars)`);
      ...
      if (!user?.id) {
        console.error(`${LOG} ❌ user.id is missing! formData will send an empty submitterId.`);
      }

      console.log(`${LOG} 🚀 Submitting job... (user.id=${user?.id}, visibility=${visibility})`);
      const { job } = await submitJob(uploadForm);
      console.log(`${LOG} ✅ Job created: id=${job.id}, status=${job.status}`);

      if (!isMountedRef.current) return;
      setJobId(job.id);

      console.log(`${LOG} ⏳ Polling until ready: jobId=${job.id}`);
      await pollUntilReady(job.id);
      console.log(`${LOG} ✅ Job is ready for chunking!`);
      ...
      console.log(`${LOG} ✂️  Spawning chunker worker...`);
      await runChunkerWorker(dataFile, job.id, chunkerCode);
      console.log(`${LOG} ✅ Chunker worker finished!`);

      if (!isMountedRef.current) return;

      setStage('complete');
      console.log(`${LOG} 🎉 Job submission complete! jobId=${job.id}`);
      if (onJobSubmitted) {
      ...
      console.error(`${LOG} ❌ handleSubmit error:`, err.message);
  ...
  // Module worker — Vite bundles it correctly via new URL + import.meta.url.
  // It imports uploadChunk from api.js directly, so no serverUrl is needed.
  const runChunkerWorker = (file, jobId, chunkerCode) => {
    return new Promise((resolve, reject) => {
      ...
      worker.postMessage({ file, jobId, chunkerCode });

      worker.onmessage = (e) => {
        if (!isMountedRef.current) return;

        const { type } = e.data;
        console.log(`${LOG} [chunkerWorker] message: type=${type}`, e.data);
        ...
        switch (type) {
          ...
          case 'done':
            console.log(`${LOG} ✅ All ${e.data.totalChunks} chunks uploaded`);
            worker.terminate();
          ...
          case 'error':
            console.error(`${LOG} ❌ Chunker worker error:`, e.data.message);
            worker.terminate();
            workerRef.current = null;
          ...
          default:
            console.warn(`${LOG} [chunkerWorker] unknown message type: ${type}`);
            break;
        }
      };

      worker.onerror = (err) => {
        console.error(`${LOG} ❌ Worker uncaught error:`, err.message, err);
        worker.terminate();
        workerRef.current = null;
  ...
  const pollUntilReady = (jobId) => {
    return new Promise((resolve, reject) => {
      ...
      const poll = async () => {
        if (!isMountedRef.current) {
          console.log(`${LOG} [poll] component unmounted, stopping poll`);
          return;
        }

        const elapsed = Date.now() - startedAt;
        if (elapsed > HARD_TIMEOUT_MS) {
          console.error(`${LOG} [poll] ❌ Hard timeout after ${HARD_TIMEOUT_MS / 1000}s`);
        ...
        try {
          ...
          console.log(`${LOG} [poll] job.status=${job.status} (elapsed ${Math.round(elapsed / 1000)}s)`);

          // BUG FIX: also accept 'distributing' — job may skip 'ready' if chunks
          // are already cached (sourceHash match). Without this the poll would
          // spin until the 90s timeout fires.
          if (job.status === 'ready' || job.status === 'distributing') {
            console.log(`${LOG} [poll] ✅ Job is ready! (status=${job.status})`);
            resolve();
            return;
          }
          if (job.status === 'failed') {
            console.error(`${LOG} [poll] ❌ Job failed:`, job.errorDetail);
            reject(new Error(job.errorDetail || 'Compilation failed'));
            return;
          }
          if (job.status === 'complete') {
            console.warn(`${LOG} [poll] job already complete — was submitted with cached WASM?`);
            resolve();
            return;
          }
          ...
          console.error(`${LOG} [poll] fetch error:`, err.message);
          reject(err);
        }
      };
    ...
    >
      ...
      >
        ...
        {jobId && (
          ...
          >
            <p
              className="text-sm font-semibold tracking-wide mb-2 uppercase"
      ...
      {stages.map((s, i) => (
        <div key={s.key} className="flex items-center flex-1">
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm text-neutral-400 font-semibold mt-2 uppercase tracking-wide">
              {s.label}
            </span>
          </div>
          {i < stages.length - 1 && (
