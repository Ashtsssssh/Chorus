...
### IN PROGRESS: Part 4 — Backend State & WebSocket Layer 🔄

**Implementation Location:** `server/src/services/sse_manager.js`

**Status:** SSE (Server-Sent Events) framework partially implemented

**Features (Planned vs Actual):**
- ✅ Job state persisted in MongoDB
- ✅ Express routes for job queries
- ⚠️ Worker registry in memory (TODO: Add worker heartbeat)
- 🔄 SSE event stream for chunk completion (schema defined, not fully wired)
- ⚠️ SSE broadcasting to multiple clients (TODO: Implement)

**Remaining Work:**
1. Implement SSE endpoint: GET `/api/jobs/:jobId/events`
2. Wire chunk completion events to SSE broadcast
3. Client-side SSE listener in Submitter component (for progress updates)
4. Worker heartbeat mechanism (TTL timers)
...
#### 4. No WASM Binary Caching Strategy
**Issue:** Each worker downloads .wasm binary independently; no shared cache.  
**Impact:** Wasted bandwidth if 1000 workers process same job.  
**Severity:** MEDIUM (infrastructure cost)  
**Fix (Part 8):** Implement Redis-based cache; workers check cache before download.
...
#### 6. No Numerical Epsilon Handling
**Issue:** If algorithm involves floating-point math, results may differ by rounding (even on identical hardware).  
**Impact:** Verification logic (Part 6) will fail due to "different" hashes.  
**Severity:** MEDIUM (only affects numerical workloads)  
**Fix (Part 8):** Add epsilon-based comparison for float-heavy results.
